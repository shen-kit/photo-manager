...
import { AlertTriangle, LoaderCircle, Search, Workflow } from "lucide-react";
...
export function SearchPage() {
  ...
  const searchQuery = useQuery({
    ...
    queryFn: () => searchAssets(normalizedQuery, SEARCH_LIMIT, 0),
    enabled: accessReady && normalizedQuery.length > 0,
  });
  ...
  return (
    <>
      <AppShell
        ...
        title="CLIP Search"
        description="Test semantic search and queue embedding backfills."
        headerActions={
          <button
            type="button"
        ...
        <section className="rounded-[28px] border border-white/10 bg-black/25 p-5 shadow-panel backdrop-blur">
          <div className="mb-5 flex flex-col gap-3 border-b border-white/10 pb-4">
            <div>
              ...
              <p className="text-sm text-slate-400">
                Enter a natural language query to test OpenCLIP-backed search.
              </p>
            </div>
            ...
            <form
              ...
              onSubmit={(event) => {
                ...
                if (!nextQuery) {
                  pushToast("Enter a search query first.", "error");
                  setSubmittedQuery("");
                  return;
                }
          ...
          {normalizedQuery ? (
            <div className="mb-5 text-sm text-slate-400">
              {searchQuery.data ? `${searchQuery.data.total} results for "${searchQuery.data.query}"` : `Searching for "${normalizedQuery}"`}
            </div>
          ) : null}
                  ...
                  >
                    <div className="relative aspect-square overflow-hidden">
                      <div
                        ...
                        style={placeholderStyle(asset.blurhash ?? asset.id)}
                      />
                      <img
