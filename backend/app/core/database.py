...
SCHEMA_PATCHES: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    (
        "ALTER TABLE assets ADD COLUMN IF NOT EXISTS has_large_preview BOOLEAN NOT NULL DEFAULT false",
        ("assets",),
    ),
    (
        "ALTER TABLE assets ADD COLUMN IF NOT EXISTS captured_at_local TEXT",
        ("assets", "ai_models"),
    ),
    ...
    (
        "ALTER TABLE assets ADD COLUMN IF NOT EXISTS duration_seconds DOUBLE PRECISION",
        ("assets",),
    ),
    ...
    (
        "ALTER TABLE assets ADD COLUMN IF NOT EXISTS search_vector vector(512)",
        ("assets",),
    ),
    (
        "ALTER TABLE assets ADD COLUMN IF NOT EXISTS search_model_id INTEGER REFERENCES ai_models(id)",
        ("assets", "ai_models"),
    ),
    ...
    (
        "ALTER TABLE people ADD COLUMN IF NOT EXISTS thumbnail_face_id UUID REFERENCES faces(id)",
        ("people", "faces"),
    ),
    (
    ...
    (
        "ALTER TABLE faces ADD COLUMN IF NOT EXISTS face_model_id INTEGER REFERENCES ai_models(id)",
        ("faces", "ai_models"),
    ),
    (
        "ALTER TABLE faces ADD COLUMN IF NOT EXISTS is_confirmed BOOLEAN NOT NULL DEFAULT false",
        ("faces",),
    ),
    (
        "ALTER TABLE faces ADD COLUMN IF NOT EXISTS is_excluded BOOLEAN NOT NULL DEFAULT false",
        ("faces",),
    ),
    (
...
def _apply_schema_patches(connection) -> None:
    for statement, dependencies in SCHEMA_PATCHES:
        if any(
            not _table_exists(connection, dependency) for dependency in dependencies
        ):
            continue
